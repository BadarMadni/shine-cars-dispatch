...
const STAT_CONFIG = [
  ...
  { key: "commission", label: "Commission", icon: TrendingUp, gradient: "from-crimson to-crimson-dark", bg: "bg-red-50", color: "text-crimson" },
  { key: "netEarned", label: "Net Earned", icon: TrendingUp, gradient: "from-emerald-500 to-emerald-600", bg: "bg-emerald-50", color: "text-emerald-600" },
];

export default function DriverBookingStats({ bookings, commissionRate = 20 }: { bookings: Booking[]; commissionRate?: number }) {
  const counts: Record<string, number> = {
    ...
    commission: Math.round(bookings.filter((b) => b.status === "completed").reduce((sum, b) => sum + (b.fare || 0), 0) * (commissionRate / 100) * 100) / 100,
    netEarned: Math.round(bookings.filter((b) => b.status === "completed").reduce((sum, b) => sum + (b.fare || 0), 0) * (1 - commissionRate / 100) * 100) / 100,
  };
      ...
      {STAT_CONFIG.map(({ key, label, icon: Icon, gradient, bg, color }) => (
        <div key={key} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 group hover:shadow-md transition-shadow">
          ...
          <p className={`text-2xl font-bold ${color}`}>
            {["earnings", "commission", "netEarned"].includes(key) ? `£${counts[key].toFixed(2)}` : counts[key]}
          </p>
