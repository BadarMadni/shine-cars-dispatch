import Sidebar from "@/components/dispatch/Sidebar";
import NewBookingAlert from "@/components/dispatch/NewBookingAlert";
import SIPPhone from "@/components/dispatch/SIPPhone";

export default function PanelLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-screen bg-gray-50/80 overflow-x-hidden w-full">
      <Sidebar />
      <main className="flex-1 lg:ml-64 min-h-screen pt-14 lg:pt-0 min-w-0 overflow-x-hidden">
        {children}
      </main>
      <NewBookingAlert />
      <SIPPhone />
    </div>
  );
}
